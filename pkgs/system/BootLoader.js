...
export default {
  ...
  exec: async function (Root) {
    let appData = await (await fetch("appData.json")).json();
    console.log(appData);
    ...
    // Make app
    let ls;
    ls = await Root.Core.startPkg("ui:LoadingScreen");

    if ("splashScreen" in appData) {
      ls = await Root.Core.startPkg(appData.splashScreen);
    }
    ...
    try {
      ...
      if (await vfs.exists("Root/Pluto/config/settingsConfig.json")) {
        ...
        if (
          ...
          await Root.Core.startPkg(appData.pkgId, true, true);
        }
      } else {
        await Root.Core.startPkg("apps:FTGSF");
        await Root.Core.startPkg(appData.pkgId, true, true);
